...
* Return value:
*********************************************************************************/
static void Crc (const uint8_t poly [], uint8_t polyLength, const uint8_t seed [], const uint8_t data [], uint16_t dataLength, uint8_t crc [])
{    
	uint16_t dataIndex;
	uint8_t  crcIndex; 
	uint8_t  bitCount;
	...
	for (dataIndex = 0; dataIndex < dataLength; ++dataIndex) { 
		...
		for (bitCount = 0; bitCount < BITS_PER_BYTE; ++bitCount) {
			...
			if ((crc[0] & MSB) != 0) {
				/* left-shift bits in all bytes of CRC except for least-significant byte */
				for (crcIndex = 0; crcIndex < (polyLength - 1); ++crcIndex) {
					/* shift in MSB from next least-significant byte to LSB of current byte */
					crc[crcIndex] = ((uint8_t)((crc[crcIndex] << 1) & 0xff)) | ((crc[crcIndex + 1] & MSB) >> (BITS_PER_BYTE - 1));
...
/*********************************************************************************
* Function name:	CrcCcittTypeToBytes	
*
* Description:
*
* Input parameters:
...
* Return value:
*********************************************************************************/ 
void CrcCcittTypeToBytes (CrcCcittType type, uint8_t bytes [])
{
...
/*********************************************************************************
* Function name:	BytesToCrcCcittType	
*
* Description:
*
* Input parameters:
...
/*********************************************************************************
* Function name:	Crc32TypeToBytes	
*
* Description:
*
* Input parameters:
...
* Return value:
*********************************************************************************/ 
void Crc32TypeToBytes (Crc32Type type, uint8_t bytes [])
{
...
/*********************************************************************************
* Function name:	BytesToCrc32Type	
*
* Description:
*
* Input parameters:
...
* Return value:
*********************************************************************************/
CrcCcittType CrcCcitt (const uint8_t data [], uint16_t length, const CrcCcittType seed)
{   
	uint8_t crc [CRC_CCITT_LENGTH];

	CrcCcittTypeToBytes(seed, crc);

	Crc(CRC_CCITT_POLY, CRC_CCITT_LENGTH, crc, data, length, crc);	
...
* Return value:
*********************************************************************************/
Crc32Type Crc32 (const uint8_t data [], uint16_t length, const Crc32Type seed)
{    
	uint8_t crc [CRC_32_LENGTH];

	Crc32TypeToBytes(seed, crc);

	Crc(CRC_32_POLY, CRC_32_LENGTH, crc, data, length, crc);
